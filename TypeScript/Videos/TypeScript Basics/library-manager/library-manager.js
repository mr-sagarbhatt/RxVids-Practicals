...
// * ADD BOOKS
function addBook(objAllBooks, objBook) {
    objAllBooks.push(objBook);
}
function getBookTitlesByCategory(categoryFilter = enums_1.Category.Fiction) {
    console.log(`Getting books in category: ${enums_1.Category[categoryFilter]}(${categoryFilter})`);
...
const poetryBooks = getBookTitlesByCategory(enums_1.Category.Poetry);
...
function fnCreateCustomer(name = "sagar", age, city) {
...
const myBooks = checkoutBooks("sagar", 1, 3);
...
const favoriteLibrarian = new classes_1.UniversityLibrarian();
...
const refBook = new classes_1.Encyclopedia("WorldPedia", 1900, 10);
refBook.printItem();
